...
# Paste ALL the hex values from your .h file inside these quotes. 
# (Note: I removed the first 6 metadata bytes '8,16,32,96,2,56' from the start)
raw_hex_data = """
...
# Clean up the string and convert it into a list of strings
clean_data = raw_hex_data.replace('\n', '').replace('\t', '').replace(' ', '')
hex_list = [h for h in clean_data.split(',') if h != '']

# Open a new .mif file to write to
with open("font8x16.mif", "w") as f:
    # Write the Quartus .mif header
    f.write(f"DEPTH = {len(hex_list)};\n")
    f.write("WIDTH = 8;\n") # 8 bits wide per line
    f.write("ADDRESS_RADIX = HEX;\n")
    f.write("DATA_RADIX = HEX;\n\n")
    f.write("CONTENT BEGIN\n")
    
    # Write each memory address and its corresponding hex value
    for address, hex_val in enumerate(hex_list):
        # Format: [Address in Hex] : [Data in Hex];
        # Strip the '0x' from the data
        formatted_val = hex_val.replace('0x', '') 
        f.write(f"\t{address:04X} : {formatted_val};\n")
        
    f.write("END;\n")

print(f"Successfully created font8x16.mif with {len(hex_list)} bytes of memory!")